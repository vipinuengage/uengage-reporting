...
const syncRiderLogsCron = () => {
  ...
  schedule(CRON_EXPRESSIONS.syncRiderLogsCronExp, async () => {
    ...
    try {
      ...
      const startTimestamp = !lastInsertedAt || moment(lastInsertedAt).isBefore(todayStart) ? todayStartStr : moment(lastInsertedAt).format("YYYY-MM-DD HH:mm:ss");
      ...
      for (let batchIndex = 0; batchIndex < totalBatches; batchIndex++) {
        ...
        const batchPromises = currentBatch.map(async (fmi) => {
          try {
            ...
            await report_db_connection.execute(`INSERT INTO rider_logs (${fields.join(",")}) VALUES (${placeholders})`, values);
          } catch (err) {
